...
export function WalletView({ isOpen, onClose, currentUser }: WalletViewProps) {
    const [view, setView] = useState<'balance' | 'store'>('balance')
    ...
    const packages: TokenPackage[] = [
        ...
        { id: 'pkg-2', name: 'Prata', tokens: 30, bonus: 5, price: 25.00, color: 'from-slate-300 to-slate-500', popular: true },
        { id: 'pkg-3', name: 'Ouro', tokens: 65, bonus: 15, price: 50.00, color: 'from-yellow-400 to-yellow-600' },
    ...
    const handleBuyPackage = (pkg: TokenPackage) => {
        alert(`Finalizando compra do pacote ${pkg.name}...\n\nVocê receberá ${pkg.tokens} ₮ (incluindo bônus) por R$ ${pkg.price.toFixed(2).replace('.', ',')}\n\n(Aguardando integração com o Checkout Pix)`)
    }
                ...
                <CardHeader className="border-b border-border/50 pb-4 bg-secondary/10 shrink-0">
                    <div className="flex items-center justify-between">
                        <CardTitle className="text-xl font-bold flex items-center gap-2">
                            {view === 'store' ? (
                                <button onClick={() => setView('balance')} className="flex items-center gap-2 hover:text-primary transition-colors">
                ...
                <CardContent className="flex-1 overflow-hidden flex flex-col p-0 min-h-0">
                    {view === 'balance' ? (
                            ...
                            <div className="bg-gradient-to-br from-primary/20 via-primary/5 to-transparent rounded-[24px] p-8 flex flex-col items-center justify-center border border-primary/20 relative overflow-hidden shadow-inner group shrink-0">
                                ...
                                <div className="grid grid-cols-2 gap-3 mt-8 w-full z-10">
                                    <Button
                                        ...
                                        className="h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-black rounded-2xl shadow-lg shadow-primary/20 uppercase italic text-xs"
                                    >
                                    ...
                                    <Button
                                        ...
                                        className="h-12 border-primary/30 bg-background/50 text-foreground font-black rounded-2xl hover:bg-primary/5 uppercase italic text-xs"
                                    >
                                                    ...
                                                    <div key={tx.id} className="flex items-center justify-between p-4 rounded-2xl bg-secondary/20 border border-border/40 hover:bg-secondary/40 transition-all group">
                                                        <div className="flex items-center gap-4">
                                                            <div className={cn(
                                                                ...
                                                                isIncoming ? "bg-green-500/10 text-green-500" : "bg-primary/10 text-primary"
                                                            )}>
                                                            ...
                                                            <div className="min-w-0">
                                                                <p className="font-bold text-sm leading-none mb-1">
                                                                    {tx.type === 'deposit' ? 'Depósito' : tx.type === 'purchase' ? 'Venda' : isIncoming ? 'Recebido' : 'Pagamento'}
                                                                </p>
                                                                <p className="text-[10px] text-muted-foreground font-medium truncate max-w-[140px]">
                                                                    {tx.description || (isIncoming ? 'Tokens recebidos' : 'Tokens enviados')}
                                                                </p>
                                                            </div>
                                                        </div>
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                            <div className="p-6 bg-primary/5 border-b border-border/10 shrink-0">
                                ...
                                <div className="grid grid-cols-1 gap-4 py-6">
                                    {packages.map((pkg) => (
                                        <Card
                                            key={pkg.id}
                                            className={cn(
                                                "relative overflow-hidden cursor-pointer border-border/50 hover:border-primary/50 transition-all hover:scale-[1.03] active:scale-[0.98] group rounded-2xl",
                                                pkg.popular && "ring-2 ring-primary border-primary/50 shadow-xl shadow-primary/10",
                                                pkg.highlight && "ring-2 ring-cyan-400 border-cyan-400/50 shadow-2xl shadow-cyan-400/20"
                                            )}
                                            onClick={() => handleBuyPackage(pkg)}
                                        >
                                            {pkg.popular && (
                                                <div className="absolute top-0 right-0">
                                                    <Badge className="rounded-none rounded-bl-lg bg-primary text-primary-foreground font-black text-[9px] px-3 py-1 italic">MAIS VENDIDO</Badge>
                                                </div>
                                            )}
                                            {pkg.highlight && (
                                                <div className="absolute top-0 right-0">
                                                    <Badge className="rounded-none rounded-bl-lg bg-cyan-400 text-black font-black text-[9px] px-3 py-1 italic">MELHOR CUSTO-BENEFÍCIO</Badge>
                                                </div>
                                            )}
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex flex-col">
                                                            ...
                                                            {pkg.bonus > 0 && (
                                                                <span className="text-[9px] font-black text-green-500 uppercase">Inclui bônus de {pkg.bonus} ₮</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <div className="p-5 rounded-[24px] bg-secondary/20 border border-dashed border-border/60 flex items-center gap-4 mt-2">
                                        ...
                                        <p className="text-[10px] font-medium text-muted-foreground leading-snug">
                                            A transação é criptografada e segura. Seus tokens serão creditados instantaneamente após o pagamento.
                                        </p>
                                    </div>
